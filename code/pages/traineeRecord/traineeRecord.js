...
*/
   ...
   * 获取学员历史记录
   */
  getWorkoutCalender: function (startDate, endDate, dateList) {
    var __this__ = this;
    var app = getApp();

    wx.request({
      url: app.globalData.host + '/fitness/V2/Mover/getWorkoutCalender?moverid=' + __this__.data.moverid + '&from_day=' + startDate + '&to_day=' + endDate,
      method: 'GET',
      dataType: 'json',
      success: function (res) {
        ...
        if (res.data.errorcode === 0) {
          ...
          calendar.forEach(function (v, k) {
            v.workouts.forEach(function (m, n) {
            ...
            if (v.workouts && v.workouts.length > 0) {
          ...
          var selectedDayWorkout = calendar.filter(function (v, i, arr) {
  ...
  getDateList: function (y, mon) {
    ...
    this.getWorkoutCalender(start, end, dateList);
  },
  selectDate: function (e) {
    var vm = this;
    // console.log('选中', e.currentTarget.dataset.date.value);

    var selectedDayWorkout = vm.data.calendar.filter(function (v, i, arr) {
